...
static void D3D_UpdatePresentParams(SDL_Window *window, D3DPRESENT_PARAMETERS *pparams)
{
    int w, h;
    ...
    if ((window_flags & SDL_WINDOW_FULLSCREEN_MASK) == SDL_WINDOW_FULLSCREEN) {
...
static int D3D_ActivateRenderer(SDL_Renderer *renderer)
{
    ...
    if (data->updateSize) {
        D3D_UpdatePresentParams(renderer->window, &data->pparams);
...
static int D3D_QueueDrawPoints(SDL_Renderer *renderer, SDL_RenderCommand *cmd, const SDL_FPoint *points, int count)
{
    ...
    const D3DCOLOR color = SDL_Swap32(SDL_RolLE32(SDL_ColorRGBAmask(&cmd->data.draw.color), 8));
                             ...
                             float scale_x, float scale_y)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        verts->color = SDL_Swap32(SDL_RolLE32(SDL_ColorRGBAmask(&col_), 8));
...
static int D3D_RunCommandQueue(SDL_Renderer *renderer, SDL_RenderCommand *cmd, void *vertices, size_t vertsize)
{
    ...
    while (cmd) {
        ...
        case SDL_RENDERCMD_CLEAR:
        {
            ...
            const D3DCOLOR color = SDL_Swap32(SDL_RolLE32(SDL_ColorRGBAmask(&cmd->data.color.color), 8));
            const SDL_Rect *viewport = &data->drawstate.viewport;
