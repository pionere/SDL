...
#define D3D11_SetError(msg, result) WIN_SetErrorFromHRESULT(SDL_COMPOSE_ERROR(msg), result)
...
static ID3D11BlendState *D3D11_CreateBlendState(D3D11_RenderData *data, SDL_BlendMode blendMode)
{
    ...
    if (FAILED(result)) {
        D3D11_SetError("ID3D11Device1::CreateBlendState", result);
        return NULL;
    }
...
static HRESULT D3D11_CreateDeviceResources(SDL_Renderer *renderer)
{
    ...
    if (FAILED(result)) {
        D3D11_SetError("CreateDXGIFactory", result);
        goto done;
    }

//    result = IDXGIFactory2_EnumAdapters(data->dxgiFactory, 0, &data->dxgiAdapter);
//    if (FAILED(result)) {
//        D3D11_SetError("D3D11CreateDevice", result);
//        goto done;
//    }
    ...
    if (FAILED(result)) {
        D3D11_SetError("IDXGIDevice1::SetMaximumFrameLatency", result);
        goto done;
    }
    ...
    if (FAILED(result)) {
        D3D11_SetError("ID3D11Device1::CreateBuffer [vertex shader constants]", result);
        goto done;
    }
    ...
    if (FAILED(result)) {
        D3D11_SetError("ID3D11Device1::CreateSamplerState [nearest-pixel filter]", result);
        goto done;
    }
    ...
    result = ID3D11Device_CreateSamplerState(data->d3dDevice,
                                             ...
                                             &data->linearSampler);
    if (FAILED(result)) {
        D3D11_SetError("ID3D11Device1::CreateSamplerState [linear filter]", result);
        goto done;
    }
    ...
    if (FAILED(result)) {
        D3D11_SetError("ID3D11Device1::CreateRasterizerState [main rasterizer]", result);
        goto done;
    }
    ...
    if (FAILED(result)) {
        D3D11_SetError("ID3D11Device1::CreateRasterizerState [clipped rasterizer]", result);
        goto done;
    }

    /* Create blending states: */
    if (!D3D11_CreateBlendState(data, SDL_BLENDMODE_BLEND) ||
        !D3D11_CreateBlendState(data, SDL_BLENDMODE_ADD) ||
...
static HRESULT D3D11_CreateSwapChain(SDL_Renderer *renderer)
{
    ...
    if (data->swapChain) {
        ...
        if (FAILED(result)) {
            if (result == DXGI_ERROR_DEVICE_REMOVED) {
                ...
                D3D11_SetError("IDXGISwapChain::ResizeBuffers", result);
            }
        }
#endif
        return result;
    ...
    if (coreWindow) {
        result = IDXGIFactory2_CreateSwapChainForCoreWindow(data->dxgiFactory,
                                                            ...
                                                            &data->swapChain);
        if (FAILED(result)) {
            D3D11_SetError("IDXGIFactory2::CreateSwapChainForCoreWindow", result);
            goto done;
        }
    } else if (usingXAML) {
        result = IDXGIFactory2_CreateSwapChainForComposition(data->dxgiFactory,
                                                             ...
                                                             &data->swapChain);
        if (FAILED(result)) {
            D3D11_SetError("IDXGIFactory2::CreateSwapChainForComposition", result);
            goto done;
        }
        ...
        if (FAILED(result)) {
            D3D11_SetError("ISwapChainBackgroundPanelNative::SetSwapChain", result);
            goto done;
        }
#else
...
#error "VideoDeviceId must be checked in D3D11_CreateSwapChain"
        ...
        result = IDXGIFactory2_CreateSwapChainForHwnd(data->dxgiFactory,
                                                      ...
                                                      &data->swapChain);
        if (FAILED(result)) {
            D3D11_SetError("IDXGIFactory2::CreateSwapChainForHwnd", result);
            goto done;
        }
...
static void D3D11_ReleaseMainRenderTargetView(D3D11_RenderData *data)
{
...
//        if (data->swapEffect == DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL) {
            ...
            if (FAILED(result)) {
                D3D11_SetError("IDXGISwapChain1::SetRotation", result);
                goto done;
            }
//        }
    }
    ...
    result = IDXGISwapChain_GetBuffer(data->swapChain,
                                      ...
                                      (void **)&backBuffer);
    if (FAILED(result)) {
        D3D11_SetError("IDXGISwapChain::GetBuffer [back-buffer]", result);
        goto done;
    }
    ...
    if (FAILED(result)) {
        D3D11_SetError("ID3D11Device::CreateRenderTargetView", result);
        goto done;
    }
...
void D3D11_Trim(SDL_Renderer *renderer)
{
    ...
    if (FAILED(result)) {
        // D3D11_SetError("ID3D11Device to IDXGIDevice3", result);
        return;
    }
...
static int D3D11_CreateTexture(SDL_Renderer *renderer, SDL_Texture *texture)
{
    ...
    if (FAILED(result)) {
        D3D11_SetError("ID3D11Device1::CreateTexture2D", result);
        goto error;
    }
#if SDL_HAVE_YUV
    ...
    if (FAILED(result)) {
        D3D11_SetError("ID3D11Device1::CreateShaderResourceView", result);
        goto error;
    }
#if SDL_HAVE_YUV
    ...
    if (texture->access & SDL_TEXTUREACCESS_TARGET) {
        ...
        if (FAILED(result)) {
            D3D11_SetError("ID3D11Device1::CreateRenderTargetView", result);
            goto error;
        }
    }
...
static int D3D11_UpdateTextureInternal(SDL_Renderer *renderer, ID3D11Texture2D *texture, int bpp, int x, int y, int w, int h, const void *pixels, int pitch)
{
    ...
    result = ID3D11Device_CreateTexture2D(rendererData->d3dDevice,
                                          ...
                                          &stagingTexture);
    if (FAILED(result)) {
        return D3D11_SetError("ID3D11Device1::CreateTexture2D [create staging texture]", result);
    }
    ...
    if (FAILED(result)) {
        ...
        return D3D11_SetError("ID3D11DeviceContext1::Map [map staging texture]", result);
    }
                               ...
                               int srcPitch)
{
    ...
    if (status >= 0) {
        if (textureData->yuv_planes == SDL_D3D11_YUV_3PLANES) {
            ...
            status = D3D11_UpdateTextureInternal(renderer, format == SDL_PIXELFORMAT_YV12 ? textureData->mainTextureV : textureData->mainTextureU, bpp, rect->x / 2u, rect->y / 2u, (rect->w + 1) / 2u, (rect->h + 1) / 2u, srcPixels, (srcPitch + 1) / 2u);
            if (status >= 0) {
                /* Skip to the correct offset into the next texture */
                srcPixels = (const void *)((const Uint8 *)srcPixels + ((rect->h + 1) / 2u) * ((srcPitch + 1) / 2u));
                status = D3D11_UpdateTextureInternal(renderer, format == SDL_PIXELFORMAT_YV12 ? textureData->mainTextureU : textureData->mainTextureV, bpp, rect->x / 2u, rect->y / 2u, (rect->w + 1) / 2u, (rect->h + 1) / 2u, srcPixels, (srcPitch + 1) / 2u);
            }
        } else if (textureData->yuv_planes == SDL_D3D11_YUV_2PLANES) {
            /* Skip to the correct offset into the next texture */
            srcPixels = (const void *)((const Uint8 *)srcPixels + rect->h * srcPitch);

            status = D3D11_UpdateTextureInternal(renderer, textureData->mainTextureU, 2, rect->x / 2u, rect->y / 2u, ((rect->w + 1) / 2u), (rect->h + 1) / 2u, srcPixels, 2 * ((srcPitch + 1) / 2u));
        }
    }
#endif /* SDL_HAVE_YUV */
    return status;
                                  ...
                                  const Uint8 *Vplane, int Vpitch)
{
    ...
    if (status >= 0) {
        status = D3D11_UpdateTextureInternal(renderer, textureData->mainTextureU, bpp, rect->x / 2u, rect->y / 2u, rect->w / 2u, rect->h / 2u, Uplane, Upitch);
        if (status >= 0) {
            status = D3D11_UpdateTextureInternal(renderer, textureData->mainTextureV, bpp, rect->x / 2u, rect->y / 2u, rect->w / 2u, rect->h / 2u, Vplane, Vpitch);
        }
    }
    return status;
}
                                 ...
                                 const Uint8 *UVplane, int UVpitch)
{
    ...
    if (status >= 0) {
        status = D3D11_UpdateTextureInternal(renderer, textureData->mainTextureU, 2, rect->x / 2u, rect->y / 2u, ((rect->w + 1) / 2u), (rect->h + 1) / 2u, UVplane, UVpitch);
    }
    return status;
}
#endif
                                    ...
                                    const void *vertexData, size_t dataSizeInBytes)
{
    ...
    if (buffer->vertexBufferSize >= dataSizeInBytes) {
        ...
        result = ID3D11DeviceContext_Map(rendererData->d3dContext,
                                         ...
                                         &mappedResource);
        if (FAILED(result)) {
            return D3D11_SetError("ID3D11DeviceContext1::Map [vertex buffer]", result);
        }
        ...
        if (FAILED(result)) {
            ...
            return D3D11_SetError("ID3D11Device1::CreateBuffer [vertex buffer]", result);
        }
...
static ID3D11RenderTargetView *D3D11_GetCurrentRenderTargetView(D3D11_RenderData *data)
{
    if (data->currentOffscreenRenderTargetView) {
...
{
    ID3D11RenderTargetView *renderTargetView = D3D11_GetCurrentRenderTargetView(rendererData);
    ...
    if (blendMode != SDL_BLENDMODE_NONE) {
        ...
        if (!blendState) {
            blendState = D3D11_CreateBlendState(rendererData, blendMode);
            if (!blendState) {
                return -1;
            }
        }
...
static int D3D11_SetCopyState(D3D11_RenderData *rendererData, const SDL_RenderCommand *cmd)
{
...
static void D3D11_DrawPrimitives(D3D11_RenderData *rendererData, D3D11_PRIMITIVE_TOPOLOGY primitiveTopology, const size_t vertexStart, const size_t vertexCount)
{
...
#endif
    if (D3D11_UpdateVertexBuffer(rendererData, vertices, vertsize) < 0) {
        return -1;
    }
    ...
    while (cmd) {
        ...
        case SDL_RENDERCMD_CLEAR:
        {
            const float colorRGBA[] = {
                (cmd->data.color.color.r / 255.0f),
                (cmd->data.color.color.g / 255.0f),
                (cmd->data.color.color.b / 255.0f),
                (cmd->data.color.color.a / 255.0f)
            };
            ID3D11DeviceContext_ClearRenderTargetView(rendererData->d3dContext, D3D11_GetCurrentRenderTargetView(rendererData), colorRGBA);
            break;
        }
        ...
        default:
            SDL_assume(!"Unknown d3d11render-command");
            break;
        }
                                  ...
                                  Uint32 format, void *pixels, int pitch)
{
    ...
    result = ID3D11Device_CreateTexture2D(data->d3dDevice,
                                          ...
                                          &stagingTexture);
    if (FAILED(result)) {
        D3D11_SetError("ID3D11Device1::CreateTexture2D [create staging texture]", result);
        goto done;
    }
    ...
    result = ID3D11DeviceContext_Map(data->d3dContext,
                                     ...
                                     &textureMemory);
    if (FAILED(result)) {
        D3D11_SetError("ID3D11DeviceContext1::Map [map staging texture]", result);
        goto done;
    }
...
const SDL_RenderDriver D3D11_RenderDriver = {
