...
SDL_bool NGAGE_Copy(SDL_Renderer *renderer, SDL_Texture *texture, SDL_Rect *srcrect, SDL_Rect *dstrect)
{
    return gRenderer->Copy(renderer, texture, srcrect, dstrect) ? SDL_TRUE : SDL_FALSE;
}

SDL_bool NGAGE_CopyEx(SDL_Renderer *renderer, SDL_Texture *texture, NGAGE_CopyExData *copydata)
{
    return gRenderer->CopyEx(renderer, texture, copydata) ? SDL_TRUE : SDL_FALSE;
}

SDL_bool NGAGE_CreateTextureData(NGAGE_TextureData *data, const int width, const int height)
{
    return gRenderer->CreateTextureData(data, width, height) ? SDL_TRUE : SDL_FALSE;
}
...
void NGAGE_SuspendScreenSaverInternal(SDL_bool suspend)
{
...
bool CRenderer::Copy(SDL_Renderer *renderer, SDL_Texture *texture, const SDL_Rect *srcrect, const SDL_Rect *dstrect)
{
    ...
    if (c->a != 0xFF || c->r != 0xFF || c->g != 0xFF || c->b != 0xFF) {
...
bool CRenderer::CopyEx(SDL_Renderer *renderer, SDL_Texture *texture, const NGAGE_CopyExData *copydata)
{
    ...
    if (copydata->scale_x != 1.f || copydata->scale_y != 1.f) {
        TFixed scale_x = Real2Fix(copydata->scale_x);
        TFixed scale_y = Real2Fix(copydata->scale_y);
        TFixed center_x = Int2Fix(copydata->center.x);
        TFixed center_y = Int2Fix(copydata->center.y);
    ...
    if (copydata->angle) {
        TFixed angle = Int2Fix(copydata->angle);
        TFixed center_x = Int2Fix(copydata->center.x);
        TFixed center_y = Int2Fix(copydata->center.y);

        dest == pixel_buffer_a ? dest = pixel_buffer_b : dest = pixel_buffer_a;
        ApplyRotation(dest, source, pitch, w, h, center_x, center_y, angle);
        source = dest;
    }
