...
void ApplyColorMod(void *dest, void *source, int pitch, int width, int height, SDL_Color/*SDL_FColor*/ color)
{
    TUint16 *src_pixels = static_cast<TUint16 *>(source);
    TUint16 *dst_pixels = static_cast<TUint16 *>(dest);

    TFixed rf = Int2Fix(color.r); // Real2Fix(color.r);
    TFixed gf = Int2Fix(color.g); // Real2Fix(color.g);
    TFixed bf = Int2Fix(color.b); // Real2Fix(color.b);
...
void ApplyFlip(void* dest, void* source, int pitch, int width, int height, SDL_RendererFlip flip)
{
    TUint16* src_pixels = static_cast<TUint16*>(source);
    TUint16* dst_pixels = static_cast<TUint16*>(dest);
