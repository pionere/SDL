...
static void GIP_SetMetadataDefaults(GIP_Attachment *attachment);
...
static void GIP_EnsureMetadata(GIP_Attachment *attachment)
{
...
static void GIP_SetMetadataDefaults(GIP_Attachment *attachment)
{
    ...
    if (!attachment->joystick) {
        HIDAPI_JoystickConnected(attachment->device->device, &attachment->joystick);
    }
}
    ...
    int num_bytes)
{
    ...
    /*if (attachment->joystick) {
        joystick = SDL_JoystickFromInstanceID(attachment->joystick);
    }
    if (joystick) {
    ...
    int num_bytes)
{
    ...
    if (bytes[1] == VK_LWIN) {
        SDL_PrivateJoystickButton(joystick, SDL_CONTROLLER_BUTTON_GUIDE, (bytes[0] & 0x03) ? SDL_PRESSED : SDL_RELEASED);
    }
    ...
    int num_bytes)
{
    ...
    if ((attachment->features & GIP_FEATURE_ELITE_BUTTONS) && attachment->xbe_format == GIP_BTN_FMT_XBE2_RAW) {
        if (bytes[15] & 3) {
            ...
            SDL_PrivateJoystickButton(joystick,
                attachment->paddle_idx,
                (bytes[GIP_BTN_OFFSET_XBE2] & 0x01) ? SDL_PRESSED : SDL_RELEASED);
            SDL_PrivateJoystickButton(joystick,
                attachment->paddle_idx + 1,
                (bytes[GIP_BTN_OFFSET_XBE2] & 0x02) ? SDL_PRESSED : SDL_RELEASED);
            SDL_PrivateJoystickButton(joystick,
                attachment->paddle_idx + 2,
                (bytes[GIP_BTN_OFFSET_XBE2] & 0x04) ? SDL_PRESSED : SDL_RELEASED);
            SDL_PrivateJoystickButton(joystick,
                attachment->paddle_idx + 3,
                (bytes[GIP_BTN_OFFSET_XBE2] & 0x08) ? SDL_PRESSED : SDL_RELEASED);
        }
    }
    return true;
}
...
static bool GIP_HandleCommandHidReport(
    ...
    int num_bytes)
{
    Uint8 modifiers, changed_modifiers;
    ...
    modifiers = bytes[0];
    changed_modifiers = modifiers ^ attachment->last_modifiers;
    if (changed_modifiers & 0x02) {
        SDL_SendKeyboardKey(/*attachment->keyboard, 0,*/ SDL_SCANCODE_LSHIFT, (modifiers & 0x02) ? SDL_PRESSED : SDL_RELEASED);
    }
    ...
    if (changed_modifiers & 0x04) {
        if (modifiers & 0x04) {
            ...
            SDL_SendKeyboardKey(/*attachment->keyboard, 0,*/ SDL_SCANCODE_NUMLOCKCLEAR, SDL_PRESSED);
            SDL_SendKeyboardKey(/*attachment->keyboard, 0,*/ SDL_SCANCODE_NUMLOCKCLEAR, SDL_RELEASED);
        }
    }
    ...
    if (!bytes[2] && attachment->last_key) {
        ...
        SDL_SendKeyboardKey(/*attachment->keyboard, 0,*/ attachment->last_key, SDL_RELEASED);
...
#else
            SDL_Keycode keycode = SDL_GetDefaultKeyFromScancode(attachment->last_key);
#endif
            if (keycode != SDL_SCANCODE_UNKNOWN && keycode < 0x80) {
                char text[2];
                text[0] = (char)keycode;
                text[1] = '\0';
        ...
        SDL_SendKeyboardKey(/*attachment->keyboard, 0,*/ bytes[2], SDL_PRESSED);
    ...
    int num_bytes)
{
    ...
    GIP_HandleNavigationReport(attachment, joystick, bytes, num_bytes);

    switch (attachment->attachment_type) {
    case GIP_TYPE_GAMEPAD:
    default:
        GIP_HandleGamepadReport(attachment, joystick, bytes, num_bytes);
        break;
    ...
    if (attachment->features & GIP_FEATURE_ELITE_BUTTONS) {
            ...
            (bytes[GIP_BTN_OFFSET_XBE1] & 0x10))
        {
            SDL_PrivateJoystickButton(joystick,
                attachment->paddle_idx,
                (bytes[GIP_BTN_OFFSET_XBE1] & 0x02) ? SDL_PRESSED : SDL_RELEASED);
            SDL_PrivateJoystickButton(joystick,
                attachment->paddle_idx + 1,
                (bytes[GIP_BTN_OFFSET_XBE1] & 0x08) ? SDL_PRESSED : SDL_RELEASED);
            SDL_PrivateJoystickButton(joystick,
                attachment->paddle_idx + 2,
                (bytes[GIP_BTN_OFFSET_XBE1] & 0x01) ? SDL_PRESSED : SDL_RELEASED);
            SDL_PrivateJoystickButton(joystick,
                attachment->paddle_idx + 3,
                (bytes[GIP_BTN_OFFSET_XBE1] & 0x04) ? SDL_PRESSED : SDL_RELEASED);
...
static void GIP_ReceivePacket(GIP_Device *device, const Uint8 *bytes, int num_bytes)
{
    ...
    if (is_fragment) {
        if (header.flags & GIP_FLAG_INIT_FRAG) {
            ...
            if (fragment_offset != attachment->fragment_offset) {
                SDL_LogWarn(SDL_LOG_CATEGORY_INPUT,
                    ...
                    fragment_offset, (unsigned)attachment->fragment_offset);
        ...
        attachment->fragment_timer = SDL_GetTicks64();
...
static int HIDAPI_DriverGIP_UpdateRumble(GIP_Attachment *attachment)
{
    ...
    if (attachment->rumble_state == GIP_RUMBLE_STATE_BUSY) {
        ...
        if (SDL_GetTicks64() >= (attachment->rumble_time + RUMBLE_BUSY_TIME_MS)) {
