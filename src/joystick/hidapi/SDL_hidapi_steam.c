...
//---------------------------------------------------------------------------
static bool ResetSteamController(SDL_HIDAPI_Device *dev, uint32_t *punUpdateRateUS)
{
...
static bool HIDAPI_DriverSteam_InitDevice(SDL_HIDAPI_Device *device)
{
    ...
    if (IsDongle(device->product_id)) {
        ...
        if (res < 0) {
            SDL_SetError("Failed to send ID_DONGLE_GET_WIRELESS_STATE request");
            return SDL_FALSE;
        }
...
static SDL_bool HIDAPI_DriverSteam_OpenJoystick(SDL_HIDAPI_Device *device, SDL_Joystick *joystick)
{
    ...
    if (!ResetSteamController(device, &ctx->update_rate_in_us)) {
        return SDL_FALSE;
    }
