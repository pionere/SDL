...
static SDL_Mouse *const _this = &SDL_mouse;
    ...
    if (window && !(window->flags & SDL_WINDOW_MOUSE_CAPTURE)) {
        ...
        if (confine) {
            ...
            if (SDL_IntersectRect(confine, &window_rect, &mouse_rect)) {
                x_min = mouse_rect.x;
                y_min = mouse_rect.y;
                x_max = x_min + mouse_rect.w;
                y_max = y_min + mouse_rect.h;
            }
        }
    ...
    if (clicks < 0) {
        ...
        if (clickstate) {
            if (state != SDL_RELEASED) {
                ...
                if (SDL_TICKS_AFTER(now, clickstate->last_timestamp, mouse->double_click_time) ||
                 ...
                 int w, int h, int hot_x, int hot_y)
{
    ...
    surface = SDL_CreateRGBSurfaceWithFormat(0, w, h, 0, SDL_PIXELFORMAT_ARGB8888);
    if (!surface) {
        return NULL;
    }
