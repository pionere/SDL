...
static SDL_Cursor *Wayland_CreateCursor(SDL_Surface *surface, int hot_x, int hot_y)
{
    ...
    if (cursor) {
        Wayland_VideoData *wd = &waylandVideoData;
...
static void Wayland_SetSystemCursorShape(struct SDL_WaylandInput *input, SDL_SystemCursor id)
{
    ...
    case SDL_SYSTEM_CURSOR_HAND:
        ...
        break;
    default:
        SDL_assume(!"Unknown system cursor"); /* Should never be here... */
        shape = WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_DEFAULT;
    }
...
static void Wayland_RecreateCursor(SDL_Cursor *cursor)
{
    Wayland_VideoData *vdata = &waylandVideoData;
