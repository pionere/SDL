...
int SDL_QueueAudio(SDL_AudioDeviceID devid, const void *data, Uint32 len)
{
    ...
    } else if (!device->buffer_queue) {
        return SDL_SetError("Audio device does not have a queue");
    }
...
Uint32 SDL_DequeueAudio(SDL_AudioDeviceID devid, void *data, Uint32 len)
{
    ...
    if ((len == 0) ||                                                     /* nothing to do? */
        ...
        (!device->buffer_queue)) {                                        /* not set for queueing */
...
static void prepare_audiospec(const SDL_AudioSpec *orig, SDL_AudioSpec *prepared)
{
    ...
    if (orig->format == 0) {
        ...
        if ((env == NULL) || ((prepared->format = SDL_ParseAudioFormat(env)) == 0)) {
            prepared->format = AUDIO_S16SYS; /* a reasonable default */
        }
    }
...
static SDL_AudioFormat format_list[NUM_FORMATS][NUM_FORMATS] = {
    { AUDIO_U8, AUDIO_S8, AUDIO_S16SYS, AUDIO_S16SYS ^ SDL_AUDIO_MASK_ENDIAN, AUDIO_U16SYS,
      AUDIO_U16SYS ^ SDL_AUDIO_MASK_ENDIAN, AUDIO_S32SYS, AUDIO_S32SYS ^ SDL_AUDIO_MASK_ENDIAN, AUDIO_F32SYS, AUDIO_F32SYS ^ SDL_AUDIO_MASK_ENDIAN },
    { AUDIO_S8, AUDIO_U8, AUDIO_S16SYS, AUDIO_S16SYS ^ SDL_AUDIO_MASK_ENDIAN, AUDIO_U16SYS,
      AUDIO_U16SYS ^ SDL_AUDIO_MASK_ENDIAN, AUDIO_S32SYS, AUDIO_S32SYS ^ SDL_AUDIO_MASK_ENDIAN, AUDIO_F32SYS, AUDIO_F32SYS ^ SDL_AUDIO_MASK_ENDIAN },
